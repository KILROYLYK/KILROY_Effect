...
class Ability {
    ...
    init() {
        ...
        createPoint.call(_this);
        createCapability.call(_this);
    }
    ...
    update() {
        ...
        for (let i = 0, n = _this.panel.border.surface; i < n; i++) {
            if (_this.point.position[i] !== _this.config.position[i]) {
                ...
                movePoint.call(_this, i, position);
            } else {
                _this.point.speed[i] = 0;
            }
        }
        
        drawCapability.call(_this);
    }
 ...
 * @return {object} 坐标
 */
function getCoordinate(n, ability) {
    const _this = this,
        coordinate = {
            x: 0,
            y: 0
 ...
 * @return {void}
 */
function movePoint(n, ability) {
    const _this = this,
        coordinate = getCoordinate.call(_this, n, ability);
